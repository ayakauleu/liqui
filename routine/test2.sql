--lllliquibase formatted sql
--changeset andrei:2 runOnChange:true splitStatements:false

CREATE OR REPLACE FUNCTION auth.access_token_check_test2() RETURNS VOID
    LANGUAGE plpgsql
    SECURITY DEFINER
--commented
AS
...
BEGIN
    ...
    IF NULLIF(v_app_user, '') IS NOT NULL THEN
        ...
        PERFORM SET_CONFIG('kp_context.app_name', 'New app name 2', FALSE);
